...
import { ArrowLeft, Check, Copy, HardDrive, RefreshCw, Search, Server, Settings2, Trash2, UserPlus, Users, X } from "lucide-react";
...
type Tab = "users" | "invites" | "quotas" | "audit" | "host";
...
export default function AdminClient({ publicAppUrl }: { publicAppUrl: string | null }) {
  ...
  const tabs: { id: Tab; label: string }[] = [{ id: "users", label: "Users" }, { id: "invites", label: "Invitations" }, { id: "quotas", label: "Storage limits" }, { id: "audit", label: "Activity" }, { id: "host", label: "Host" }];

  return <div className="admin-app"><aside className="admin-sidebar"><Link className="brand" href="/"><img className="brand-mark" src="/enderchest-mark.png" alt="" width={30} height={30}/><span>EnderChest</span></Link><div className="admin-side-label">Administration</div><nav className="side-nav" aria-label="Administration">{tabs.map(t => <button key={t.id} className={tab === t.id ? "active" : ""} onClick={() => { setTab(t.id); setFilter(""); }}>{t.id === "users" ? <Users size={18}/> : t.id === "invites" ? <UserPlus size={18}/> : t.id === "quotas" ? <HardDrive size={18}/> : t.id === "host" ? <Server size={18}/> : <Settings2 size={18}/>} {t.label}</button>)}</nav><Link className="admin-back" href="/"><ArrowLeft size={17}/> Back to files</Link></aside>
    <div className="admin-main"><header className="admin-top"><span>Administration</span><div className="top-actions"><ThemeToggle/><button className="icon-button" title="Refresh" onClick={() => tab === "host" ? setHostRefresh(value => value + 1) : void load(tab)}><RefreshCw size={18}/></button></div></header><main className="admin-content"><div className="admin-heading"><div><h1>{tabs.find(t => t.id === tab)?.label}</h1><p>{tab === "users" ? "Manage who can access this server." : tab === "invites" ? "Invite people to create an account." : tab === "quotas" ? "Set how much storage each user can use." : tab === "audit" ? "Recent administrative activity." : "Keep an eye on this EnderChest instance."}</p></div>{tab === "invites" && <div className="admin-create"><label>Expires in <input type="number" min="1" max="720" value={hours} onChange={e => setHours(e.target.value)}/> hours</label><button className="primary-button" onClick={createInvite} disabled={busy}><UserPlus size={17}/> New invitation</button></div>}</div>
      ...
      {tab === "host" ? <HostDashboard publicAppUrl={publicAppUrl} refreshSignal={hostRefresh}/> : <><div className="admin-toolbar"><div className="search-wrap"><Search size={17}/><input aria-label="Filter rows" placeholder={`Search ${tabs.find(t => t.id === tab)?.label.toLowerCase()}`} value={filter} onChange={e => setFilter(e.target.value)}/></div><span>{tab === "users" ? displayedUsers.length : tab === "invites" ? displayedInvites.length : tab === "quotas" ? displayedQuotas.length : displayedAudit.length} items</span></div>
